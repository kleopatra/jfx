 ...
 * This is unused, just for keeping the comments on listener-rewiring patterns!
 * 
 * @see ListCell
 * @since 9
 */
public class ListCellCommentedSkin<T> extends CellSkinBase<ListCell<T>> {
     ...
     */
    public ListCellCommentedSkin(ListCell<T> control) {
        super(control);
...
//        // only if there is some external state to clean
////        updateFixedCellSize();
//        super.dispose();
//
//        if (behavior != null) {
...
//            // if so, cannot use updateFixedCellSize because that's accessing the life parent
//            listView.fixedCellSizeProperty().removeListener(fixedCellSizeListener);
//        }
//        super.dispose();
//
...
//                // also: listens to a different listView than the one it's updating on
